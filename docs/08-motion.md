...
One layout cannot serve both. So every surface takes `data-scale="stream" | "venue"`, and the
venue variant is not "the same thing bigger" — it's a different, brutally reduced layout.
...
| Small counters ticking | **Digit rolls** at 140px — the shape change is the signal |
...
full-height block of maximum-contrast color crossing the entire element — the highest-amplitude
motion available.
...
travel, and type-scale tokens plus seven keyframe sets. Switching a surface between stream and
venue scale is one attribute:

```html
<html data-scale="venue" data-key="alpha">
```

Nothing else changes. If a component needs bespoke CSS to work at venue scale, the component is
too complicated for a venue surface — cut it instead.
