...
gold-on-purple graphics, the same commentators — keeps it.
...
`https://api.start.gg/gql/alpha`, bearer token, GraphQL. Poll the event's sets every ~15s:

```graphql
query PhaseSets($phaseGroupId: ID!) {
  phaseGroup(id: $phaseGroupId) {
    sets(sortType: STANDARD, perPage: 32) {
      nodes {
...
Emits `arcade.bracket_updated`, `arcade.set_start`, `arcade.score`, `arcade.set_end`.
...
start.gg supplying names, seeds, and round labels. Same `confidence` field as everything else:
bracket metadata is `authoritative`, live set score is `estimated` unless the operator confirms it.
...
| **Alliance GP** | During alliance selection, run a Mario Kart GP where the karts *are* the alliance captains — standings graphic uses alliance numbers and colors |
