...
shadow (see broadcast rule 2, where shadows die under a luma key).
...
All SIL OFL. **Self-host the WOFF2s** (including the Archivo variable font, since the width axis is the
...
a 20px chamfer on broadcast surfaces, 34px on the room-only side screens. It reads as a lightening
cut on a gusset plate, which is exactly what the audience spends all weekend looking at on the
robots. Rounded rectangles read as "web app"; chamfers read as "fabricated."
...
(`--tex-perf`, 13px on broadcast surfaces / 22px on the side screens), like perforated aluminum. It's
subtle enough to survive stream compression and it stops every panel from being a dead color block.
...
4. **One broadcast scale, one room scale.** The program overlay serves the stream and the venue
   projector from a single layout: the elements that must read from the back of the gym (score,
   clock, alliance blocks) are already its largest, and everything else is stream detail the room
   can ignore. Minimum type is **28px** on the overlay. The venue **side screens** (`/s/side`) are
   the one room-only surface and carry their own **72px floor** locally, with 5-7 elements, not 15.
   Full derivation in [08-motion.md](08-motion.md).
...
7. **Never encode alliance by color alone.** Always pair with position (red left, blue right,
   matching the field) *and* a text label, since ~8% of male viewers can't reliably separate them.
